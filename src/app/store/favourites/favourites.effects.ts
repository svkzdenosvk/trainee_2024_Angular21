    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.http.get<FavouriteResponse[]>(`${API_URL}/favourites`).pipe(
          map((favourites) =>
            FavouritesActions.loadFavouritesSuccess({ favourites }),
          ),
      ...
      mergeMap(({ city }) =>
        this.http.post<FavouriteResponse>(`${API_URL}/favourites`, city).pipe(
          switchMap(() =>
            this.http.get<FavouriteResponse[]>(`${API_URL}/favourites`).pipe(
              map((favourites) =>
                FavouritesActions.addFavouriteSuccess({ favourites }),
              ),
            ),
          ),
          catchError(() =>
            of(FavouritesActions.addFavouriteFailure({ city })),
          ),
        ),
      ),
    ),
      ...
      mergeMap(({ city }) => {
        ...
        return this.http.delete(`${API_URL}/favourites/${id}`).pipe(
          switchMap(() =>
            this.http.get<FavouriteResponse[]>(`${API_URL}/favourites`).pipe(
              map((favourites) =>
                FavouritesActions.removeFavouriteSuccess({ favourites }),
              ),
            ),
          ),
          catchError(() =>
            of(FavouritesActions.removeFavouriteFailure({ city })),
          ),
        );
      }),
    ),
