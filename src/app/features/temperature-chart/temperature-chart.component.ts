...
export class TemperatureChartComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this.weatherService.getWeatherData(
      ...
      error: () => { this.error.set('Failed to load chart data.'); this.loading.set(false); }
    });
  }
  ...
  private renderChart(): void {
    ...
    const rows = this.viewMode() === 'daily'
      ? this.aggregateDailyAvg(this.allRows)
      : this.allRows;

    const labels = rows.map(r =>
      this.viewMode() === 'daily'
        ? r.datetime.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })
        : r.datetime.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })
    );
